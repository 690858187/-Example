...
//        return;
    ...
    if(param){
        dict = @{@"REQUESTS":@[@{@"REQ_COMM_DATA":param, @"REQ_MSG_HDR":@{@"SERVICE_ID":url}}]};
    }else{
        dict = @{@"REQUESTS":@[@{@"REQ_MSG_HDR":@{@"SERVICE_ID":url}}]};
    }
    
    [[PandaBaseNetManager share].http POST:default_API parameters:dict success:^(AFHTTPRequestOperation * _Nonnull operation, id  _Nonnull responseObject) {
        NSDictionary *result = responseObject;
       
        if (result) {
            
            //NSDictionary *dic = [[[result objectForKey:@"ANSWERS"] firstObject] objectForKey:@"ANS_COMM_DATA"];
